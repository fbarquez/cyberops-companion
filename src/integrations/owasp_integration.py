...
class OWASPIntegration:
    ...
    def identify_risks_from_indicators(self, indicators: List[str]) -> List[OWASPRisk]:
        """
        ...
        for risk_id, keywords in keyword_mapping.items():
            for keyword in keywords:
                if keyword in indicator_text:
                    risk = self._top_10.get(risk_id)
                    if risk and risk not in matched_risks:
    ...
    def validate_phase_compliance(
            ...
            "identified_risks": [
                {
                    "id": r.id,
                    "name": r.name,
                    "relevance": r.ir_phase_relevance.get(phase, ""),
                }
        ...
        for risk in identified_risks:
            result["recommended_actions"].extend([
                f"[{risk.id}] {action}"
                for action in risk.prevention[:2]
            ])
