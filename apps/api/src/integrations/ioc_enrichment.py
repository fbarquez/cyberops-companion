...
class IOCEnricher:
    ...
    def _query_source(
        ...
        # Real API integration would be implemented here
        return self._query_offline(ioc_value, ioc_type, source)
    ...
    def _export_markdown(self, results: List[EnrichmentResult]) -> str:
        ...
        for result in results:
            ...
            lines.append(
                ...
                f"{threat_emoji.get(result.overall_threat_level, '')} | "
                f"{result.risk_score:.1f} |"
            )
...
# Singleton instance
_ioc_enricher: Optional[IOCEnricher] = None


def get_ioc_enricher(offline_mode: bool = True) -> IOCEnricher:
    """Get the singleton IOC enricher instance."""
    global _ioc_enricher
    if _ioc_enricher is None:
        _ioc_enricher = IOCEnricher(offline_mode=offline_mode)
